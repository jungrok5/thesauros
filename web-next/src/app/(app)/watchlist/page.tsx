...
async function fetchWatchlist(email: string, name: string | null) {
  ...
  const { data, error } = await sb
    .from("watchlist")
    .select("id, ticker, category, entry_price, entry_date, note, alerts_enabled, created_at, tickers:ticker(name, market)")
    .eq("user_id", userId)
    .order("created_at", { ascending: false });
  if (error) throw new Error(error.message);
  return (data ?? []).map((r) => {
    const t = (r as { tickers?: { name?: string; market?: string } }).tickers;
    return {
      ...r,
