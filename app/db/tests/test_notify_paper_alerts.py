...
def _trade(entry: float, cur: float, **kw) -> dict:
    base = {
        "id": "trade-1",
        "user_id": "user-1",
        "ticker": "005990.KQ",
        ...
        "stop_alert_sent_at": None,
        "target_alert_sent_at": None,
    }
    base.update(kw)
    return base
...
def test_stop_message_names_the_book_rule():
    ...
    msg = _format_message("stop", _trade(entry=10_000, cur=8_700))
...
def test_target_message_recommends_partial_take_profit():
    ...
    msg = _format_message("target", _trade(entry=10_000, cur=12_500))
    assert "🎯" in msg
    assert "목표가" in msg
    assert "일부 익절" in msg
    # P&L positive
...
def test_messages_link_to_paper_page():
    ...
    stop_msg = _format_message("stop", _trade(entry=10_000, cur=8_700))
    target_msg = _format_message("target", _trade(entry=10_000, cur=12_500))
